...
from app.pdf.shared_styles import NumberedCanvas as _NumberedCanvas
...
def _build_v4_why_this_period(data: CanonicalReportData, styles) -> List:
    ...
    if w.upagraha_insight:
        elements.append(Paragraph("<b>Karmic Pattern:</b>", styles['BodyText']))
        elements.append(Paragraph(
            f"<i>{w.upagraha_insight}</i>", styles['BodyText']
        ))
        elements.append(Spacer(1, 0.1*inch))
...
def _build_upagrahas_section(data: CanonicalReportData, styles) -> List:
    ...
    rows = [["Shadow Point", "Rasi", "Lord", "Method"]]
    rows.append([
        "Gulika (Mandi)",
        gulika.get("rasi", "—"),
        gulika.get("rasi_lord", "—"),
        gulika.get("method", "parashari").replace("_", " ").title(),
    ])

...
def _build_family_member_context(member_payloads: list, porutham: Optional[dict] = None) -> str:
    ...
    for m in member_payloads:
        ...
        # Yogas + Upagraha (Gulika/Mandi) -- shared with family.py's
        # _build_member_summary() (same feature, same cost reasoning, added
        # to both in the same change). predictive_signals and kp_sublords
        # deliberately NOT extended to non-anchor family members: both
        # require either an expensive per-member build (predictive_signals
        # is time-window-specific narrative text) or a cached LLM
        # interpretation lookup (kp_sublords, natal-only) -- doing that N
        # times per family chat message would multiply DB/cost per message
        # by family size, unbounded, for the lowest-value case ("tell me
        # about my daughter's yogas"-style questions don't need her precise
        # 2026-05-17 event windows). The anchor chart (whichever chart
        # chat_stream() is running against) still gets full depth via
        # _build_chat_context()/_build_monthly_context_block() -- this skip
        # applies only to OTHER family members.
        from app.llm.payload_builder import _build_family_yoga_upagraha_suffix
        line += _build_family_yoga_upagraha_suffix(payload)
